import { DAY_NAMES, WorkoutDay, PROGRAM_DAY_ORDERS } from '@/data/exercises';
...
export function WorkoutSelectPage({
  ...
  const dayOrder = PROGRAM_DAY_ORDERS[programId] || ['push', 'pull', 'legs'];
  const nextDay = dayOrder[nextDayIndex % dayOrder.length];
  const isSingleWorkout = dayOrder.length === 1;
  ...
  return (
    <div className="px-4 pt-6 pb-24 max-w-md mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {isSingleWorkout ? 'Ready to train' : 'Choose your workout'}
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <p className="font-black text-base">
              {isSingleWorkout ? 'Start Workout' : 'Start Next Workout'}
            </p>
      ...
      {/* Individual day cards — only show if multiple days */}
      {!isSingleWorkout && (
        <div className="space-y-2">
          ...
          {dayOrder.map((day, index) => {
            const isNext = index === (nextDayIndex % dayOrder.length);
            return (
              <button
                key={day}
